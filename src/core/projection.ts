...
export interface ProjectedTag {
  /** 原始 X 坐标 / original X coordinate */
  x: number;
  /** 原始 Y 坐标 / original Y coordinate */
  y: number;
  /** 原始 Z 深度 / original Z depth */
  z: number;
  /** 缩放比例 (1 = 最近, 0 = 最远) / scale factor (1 = nearest, 0 = farthest) */
  scale: number;
  /** 透明度 (1 = 最前, 0 = 最后) / opacity (1 = front, 0 = back) */
  alpha: number;
}
 ...
 *
 * @param points — 旋转后的 3D 点 / rotated 3D points
 * @param depth — 透视深度 = 2 × 球半径 / perspective depth = 2 × sphere radius
 */
export function project(
  ...
  const d2 = 2 * depth;
  return points.map((p) => {
    // 透视缩放 / perspective scale
    const per = d2 / (d2 + p.z);
    // 透明度从 per² − 0.25 计算 / alpha derived from per² − 0.25
